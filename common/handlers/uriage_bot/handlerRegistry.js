// common/handlers/uriage_bot/handlerRegistry.js
const logger = require('../../logger');
const performanceManager = require('../../performance/performanceManager');

class UriageBotHandlerRegistry {
  constructor() {
  ...
  async initialize() {
    ...
    logger.info('[UriageBotHandlerRegistry] 🚀 Uriage Bot ハンドラーレジストリ初期化開始');

    try {
      const handlerDefinitions = [
        { name: 'uriage', path: '../../../uriage_bot/handlers/uriage_handler', priority: 1 },
        { name: 'uriageConfig', path: '../../../uriage_bot/handlers/uriageConfigHandler', priority: 3 },
      ];
      ...
      logger.info(`[UriageBotHandlerRegistry] ✅ 初期化完了: ${this.lazyHandlers.size}ハンドラー定義 (${initTime}ms)`);

    } catch (error) {
      logger.error('[UriageBotHandlerRegistry] ❌ ハンドラー初期化エラー:', error);
      throw error;
    }
  }
  ...
  async buildOptimizedRoutingTable() {
    ...
    this.precompiledPatterns = [
      { pattern: /^(uriage|売上|sales)_/i, handler: 'uriage', priority: 1 },
    ];

    const buildTime = Date.now() - buildStart;
    logger.info(`[UriageBotHandlerRegistry] ⚡ ルーティングテーブル構築完了 (${buildTime}ms)`);
  }

  async preloadCriticalHandlers() {
    const criticalHandlers = ['uriage'];

    logger.info(`[UriageBotHandlerRegistry] 🔄 重要ハンドラーのプリロード開始: ${criticalHandlers.join(', ')}`);
    ...
    const preloadTasks = criticalHandlers.map(name => async () => {
      try {
        ...
        logger.warn(`[UriageBotHandlerRegistry] プリロード失敗: ${name} - ${error.message}`);
      }
    });
  ...
  async getHandler(name) {
    ...
    try {
      ...
      logger.debug(`[UriageBotHandlerRegistry] ⚡ レイジーロード: ${name} (${loadTime}ms)`);

      return handler;

    } catch (error) {
      logger.error(`[UriageBotHandlerRegistry] ❌ ${name} 読み込みエラー:`, error);
      throw error;
    }
  }
  ...
  async routeInteractionHyperFast(interaction) {
    ...
    try {
      ...
      if (directHandler) {
        logger.debug(`[UriageBotHandlerRegistry] 直接マッチング: ${customId} -> ${directHandler}`);
      ...
      for (const [prefix, handlerName] of this.prefixRoutes) {
        if (customId.startsWith(prefix)) {
          logger.debug(`[UriageBotHandlerRegistry] プレフィックスマッチング: ${customId} -> ${handlerName} (プレフィックス: ${prefix})`);
      ...
      for (const { pattern, handler: handlerName } of this.precompiledPatterns) {
        if (pattern.test(customId)) {
          logger.debug(`[UriageBotHandlerRegistry] 正規表現マッチング: ${customId} -> ${handlerName}`);
          this.routingCache.set(cacheKey, handlerName);
      ...
      if (fallbackHandler) {
        logger.debug(`[UriageBotHandlerRegistry] フォールバック: ${customId} -> ${fallbackHandler}`);
        this.routingCache.set(cacheKey, fallbackHandler);
        return await this.executeHandler(fallbackHandler, interaction);
      }

      logger.warn(`[UriageBotHandlerRegistry] ハンドラーが見つかりません: ${customId}`);
      this.stats.routingMisses++;
      return {
        success: false,
      ...
      logger.error('[UriageBotHandlerRegistry] ルーティングエラー:', error);
      this.stats.routingMisses++;
  ...
  async executeHandler(handlerName, interaction) {
    ...
    try {
      ...
      if (!handler || typeof handler.execute !== 'function') {
        ...
        logger.error(`[UriageBotHandlerRegistry] 無効なハンドラー: ${handlerName}`);
        return {
          success: false,
      ...
      logger.debug(`[UriageBotHandlerRegistry] ✅ ${handlerName} 実行完了 (${executionTime}ms)`);
      ...
      const executionTime = Date.now() - executeStart;
      logger.error(`[UriageBotHandlerRegistry] ${handlerName} 実行エラー:`, error);
  ...
  determineFallbackHandler(interaction) {
    // Uriage Bot specific fallback logic if any, otherwise return null
    return null;
  }
...
const uriageBotHandlerRegistry = new UriageBotHandlerRegistry();
module.exports = uriageBotHandlerRegistry;
