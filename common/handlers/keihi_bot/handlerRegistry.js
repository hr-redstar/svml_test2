// common/handlers/keihi_bot/handlerRegistry.js
const logger = require('../../logger');
const performanceManager = require('../../performance/performanceManager');

class KeihiBotHandlerRegistry {
  constructor() {
  ...
  async initialize() {
    ...
    logger.info('[KeihiBotHandlerRegistry] 🚀 Keihi Bot ハンドラーレジストリ初期化開始');

    try {
      const handlerDefinitions = [
        { name: 'keihi', path: '../../../keihi_bot/handlers/keihiHandler', priority: 1 },
        { name: 'keihiConfig', path: '../../../keihi_bot/handlers/keihiConfigHandler', priority: 3 },
      ];
      ...
      logger.info(`[KeihiBotHandlerRegistry] ✅ 初期化完了: ${this.lazyHandlers.size}ハンドラー定義 (${initTime}ms)`);

    } catch (error) {
      logger.error('[KeihiBotHandlerRegistry] ❌ ハンドラー初期化エラー:', error);
      throw error;
    }
  }
  ...
  async buildOptimizedRoutingTable() {
    ...
    this.precompiledPatterns = [
      { pattern: /^(keihi|経費|expense)_/i, handler: 'keihi', priority: 1 },
    ];

    const buildTime = Date.now() - buildStart;
    logger.info(`[KeihiBotHandlerRegistry] ⚡ ルーティングテーブル構築完了 (${buildTime}ms)`);
  }

  async preloadCriticalHandlers() {
    const criticalHandlers = ['keihi'];

    logger.info(`[KeihiBotHandlerRegistry] 🔄 重要ハンドラーのプリロード開始: ${criticalHandlers.join(', ')}`);
    ...
    const preloadTasks = criticalHandlers.map(name => async () => {
      try {
        ...
        logger.warn(`[KeihiBotHandlerRegistry] プリロード失敗: ${name} - ${error.message}`);
      }
    });
  ...
  async getHandler(name) {
    ...
    try {
      ...
      logger.debug(`[KeihiBotHandlerRegistry] ⚡ レイジーロード: ${name} (${loadTime}ms)`);

      return handler;

    } catch (error) {
      logger.error(`[KeihiBotHandlerRegistry] ❌ ${name} 読み込みエラー:`, error);
      throw error;
    }
  }
  ...
  async routeInteractionHyperFast(interaction) {
    ...
    try {
      ...
      if (directHandler) {
        logger.debug(`[KeihiBotHandlerRegistry] 直接マッチング: ${customId} -> ${directHandler}`);
      ...
      for (const [prefix, handlerName] of this.prefixRoutes) {
        if (customId.startsWith(prefix)) {
          logger.debug(`[KeihiBotHandlerRegistry] プレフィックスマッチング: ${customId} -> ${handlerName} (プレフィックス: ${prefix})`);
      ...
      for (const { pattern, handler: handlerName } of this.precompiledPatterns) {
        if (pattern.test(customId)) {
          logger.debug(`[KeihiBotHandlerRegistry] 正規表現マッチング: ${customId} -> ${handlerName}`);
          this.routingCache.set(cacheKey, handlerName);
      ...
      if (fallbackHandler) {
        logger.debug(`[KeihiBotHandlerRegistry] フォールバック: ${customId} -> ${fallbackHandler}`);
        this.routingCache.set(cacheKey, fallbackHandler);
        return await this.executeHandler(fallbackHandler, interaction);
      }

      logger.warn(`[KeihiBotHandlerRegistry] ハンドラーが見つかりません: ${customId}`);
      this.stats.routingMisses++;
      return {
        success: false,
      ...
      logger.error('[KeihiBotHandlerRegistry] ルーティングエラー:', error);
      this.stats.routingMisses++;
  ...
  async executeHandler(handlerName, interaction) {
    ...
    try {
      ...
      if (!handler || typeof handler.execute !== 'function') {
        ...
        logger.error(`[KeihiBotHandlerRegistry] 無効なハンドラー: ${handlerName}`);
        return {
          success: false,
      ...
      logger.debug(`[KeihiBotHandlerRegistry] ✅ ${handlerName} 実行完了 (${executionTime}ms)`);
      ...
      const executionTime = Date.now() - executeStart;
      logger.error(`[KeihiBotHandlerRegistry] ${handlerName} 実行エラー:`, error);
  ...
  determineFallbackHandler(interaction) {
    // Keihi Bot specific fallback logic if any, otherwise return null
    return null;
  }
...
const keihiBotHandlerRegistry = new KeihiBotHandlerRegistry();
module.exports = keihiBotHandlerRegistry;
