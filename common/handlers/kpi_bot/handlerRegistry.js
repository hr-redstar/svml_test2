// common/handlers/kpi_bot/handlerRegistry.js
const logger = require('../../logger');
const performanceManager = require('../../performance/performanceManager');

class KpiBotHandlerRegistry {
  constructor() {
  ...
  async initialize() {
    ...
    logger.info('[KpiBotHandlerRegistry] 🚀 KPI Bot ハンドラーレジストリ初期化開始');

    try {
      const handlerDefinitions = [
        { name: 'kpi', path: '../../../kpi_bot/handlers/kpiHandler', priority: 1 },
      ];
      ...
      logger.info(`[KpiBotHandlerRegistry] ✅ 初期化完了: ${this.lazyHandlers.size}ハンドラー定義 (${initTime}ms)`);

    } catch (error) {
      logger.error('[KpiBotHandlerRegistry] ❌ ハンドラー初期化エラー:', error);
      throw error;
    }
  }
  ...
  async buildOptimizedRoutingTable() {
    ...
    this.precompiledPatterns = [
      { pattern: /^(kpi|目標|target)_/i, handler: 'kpi', priority: 1 },
    ];

    const buildTime = Date.now() - buildStart;
    logger.info(`[KpiBotHandlerRegistry] ⚡ ルーティングテーブル構築完了 (${buildTime}ms)`);
  }

  async preloadCriticalHandlers() {
    const criticalHandlers = ['kpi'];

    logger.info(`[KpiBotHandlerRegistry] 🔄 重要ハンドラーのプリロード開始: ${criticalHandlers.join(', ')}`);
    ...
    const preloadTasks = criticalHandlers.map(name => async () => {
      try {
        ...
        logger.warn(`[KpiBotHandlerRegistry] プリロード失敗: ${name} - ${error.message}`);
      }
    });
  ...
  async getHandler(name) {
    ...
    try {
      ...
      logger.debug(`[KpiBotHandlerRegistry] ⚡ レイジーロード: ${name} (${loadTime}ms)`);

      return handler;

    } catch (error) {
      logger.error(`[KpiBotHandlerRegistry] ❌ ${name} 読み込みエラー:`, error);
      throw error;
    }
  }
  ...
  async routeInteractionHyperFast(interaction) {
    ...
    try {
      ...
      if (directHandler) {
        logger.debug(`[KpiBotHandlerRegistry] 直接マッチング: ${customId} -> ${directHandler}`);
      ...
      for (const [prefix, handlerName] of this.prefixRoutes) {
        if (customId.startsWith(prefix)) {
          logger.debug(`[KpiBotHandlerRegistry] プレフィックスマッチング: ${customId} -> ${handlerName} (プレフィックス: ${prefix})`);
      ...
      for (const { pattern, handler: handlerName } of this.precompiledPatterns) {
        if (pattern.test(customId)) {
          logger.debug(`[KpiBotHandlerRegistry] 正規表現マッチング: ${customId} -> ${handlerName}`);
          this.routingCache.set(cacheKey, handlerName);
      ...
      if (fallbackHandler) {
        logger.debug(`[KpiBotHandlerRegistry] フォールバック: ${customId} -> ${fallbackHandler}`);
        this.routingCache.set(cacheKey, fallbackHandler);
        return await this.executeHandler(fallbackHandler, interaction);
      }

      logger.warn(`[KpiBotHandlerRegistry] ハンドラーが見つかりません: ${customId}`);
      this.stats.routingMisses++;
      return {
        success: false,
      ...
      logger.error('[KpiBotHandlerRegistry] ルーティングエラー:', error);
      this.stats.routingMisses++;
  ...
  async executeHandler(handlerName, interaction) {
    ...
    try {
      ...
      if (!handler || typeof handler.execute !== 'function') {
        ...
        logger.error(`[KpiBotHandlerRegistry] 無効なハンドラー: ${handlerName}`);
        return {
          success: false,
      ...
      logger.debug(`[KpiBotHandlerRegistry] ✅ ${handlerName} 実行完了 (${executionTime}ms)`);
      ...
      const executionTime = Date.now() - executeStart;
      logger.error(`[KpiBotHandlerRegistry] ${handlerName} 実行エラー:`, error);
  ...
  determineFallbackHandler(interaction) {
    // KPI Bot specific fallback logic if any, otherwise return null
    return null;
  }
...
const kpiBotHandlerRegistry = new KpiBotHandlerRegistry();
module.exports = kpiBotHandlerRegistry;
